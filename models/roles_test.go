...
func testRoleToManyPermissionIdPermissions(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"role_permissions\" (\"roleId\", \"permissionId\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"role_permissions\" (\"roleId\", \"permissionId\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}

	check, err := a.PermissionIdPermissions().All(ctx, tx)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadPermissionIdPermissions(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PermissionIdPermissions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
	}

	a.R.PermissionIdPermissions = nil
	if err = a.L.LoadPermissionIdPermissions(ctx, tx, true, &a, nil); err != nil {
		t.Fatal(err)
	}
	if got := len(a.R.PermissionIdPermissions); got != 2 {
		t.Error("number of eager loaded records wrong, got:", got)
...
func testRoleToManyUserIdUsers(t *testing.T) {
	var err error
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	_, err = tx.Exec("insert into \"user_roles\" (\"roleId\", \"userId\") values ($1, $2)", a.ID, b.ID)
	if err != nil {
		t.Fatal(err)
	}
	_, err = tx.Exec("insert into \"user_roles\" (\"roleId\", \"userId\") values ($1, $2)", a.ID, c.ID)
	if err != nil {
		t.Fatal(err)
	}
	...
	slice := RoleSlice{&a}
	if err = a.L.LoadUserIdUsers(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoleToManyAddOpPermissionIdPermissions(t *testing.T) {
	var err error
	...
	for i, x := range foreignersSplitByInsertion {
		err = a.AddPermissionIdPermissions(ctx, tx, i != 0, x...)
		if err != nil {
			t.Fatal(err)
		}
		...
		if a.R.PermissionIdPermissions[i*2] != first {
			t.Error("relationship struct slice not set to correct value")
		}
		if a.R.PermissionIdPermissions[i*2+1] != second {
			t.Error("relationship struct slice not set to correct value")
		}

		count, err := a.PermissionIdPermissions().Count(ctx, tx)
		if err != nil {
			t.Fatal(err)
		}
...
func testRoleToManySetOpPermissionIdPermissions(t *testing.T) {
	var err error
	...
	err = a.SetPermissionIdPermissions(ctx, tx, false, &b, &c)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.SetPermissionIdPermissions(ctx, tx, true, &d, &e)
	if err != nil {
		t.Fatal(err)
	}
	...
	if a.R.PermissionIdPermissions[0] != &d {
		t.Error("relationship struct slice not set to correct value")
	}
	if a.R.PermissionIdPermissions[1] != &e {
		t.Error("relationship struct slice not set to correct value")
	}
}

func testRoleToManyRemoveOpPermissionIdPermissions(t *testing.T) {
	var err error
	...
	err = a.AddPermissionIdPermissions(ctx, tx, true, foreigners...)
	if err != nil {
		t.Fatal(err)
	}
	...
	err = a.RemovePermissionIdPermissions(ctx, tx, foreigners[:2]...)
	if err != nil {
		t.Fatal(err)
	}
	...
	if len(a.R.PermissionIdPermissions) != 2 {
		t.Error("should have preserved two relationships")
	}

	// Removal doesn't do a stable deletion for performance so we have to flip the order
	if a.R.PermissionIdPermissions[1] != &d {
		t.Error("relationship to d should have been preserved")
	}
	if a.R.PermissionIdPermissions[0] != &e {
		t.Error("relationship to e should have been preserved")
	}
}

func testRoleToManyAddOpUserIdUsers(t *testing.T) {
	var err error
...
func testRoleToManySetOpUserIdUsers(t *testing.T) {
	var err error
...
func testRoleToManyRemoveOpUserIdUsers(t *testing.T) {
	var err error
...
func testRoleToOneTenantUsingTenantIdTenant(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadTenantIdTenant(ctx, tx, false, (*[]*Role)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testRoleToOneSetOpTenantUsingTenantIdTenant(t *testing.T) {
	var err error
...
var (
	roleDBTypes = map[string]string{`ID`: `character varying`, `Name`: `character varying`, `TenantId`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
