...
func testTeamToOneTenantUsingTenantIdTenant(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadTenantIdTenant(ctx, tx, false, (*[]*Team)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testTeamToOneSetOpTenantUsingTenantIdTenant(t *testing.T) {
	var err error
...
var (
	teamDBTypes = map[string]string{`ID`: `character varying`, `Name`: `character varying`, `TenantId`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_           = bytes.MinRead
)
