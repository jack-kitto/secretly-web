...
var RoleWhere = struct {
	...
	TenantId:  whereHelperstring{field: "\"roles\".\"tenantId\""},
	CreatedAt: whereHelpernull_Time{field: "\"roles\".\"createdAt\""},
	UpdatedAt: whereHelpernull_Time{field: "\"roles\".\"updatedAt\""},
}

// RoleRels is where relationship names are stored.
var RoleRels = struct {
	TenantIdTenant          string
	PermissionIdPermissions string
	UserIdUsers             string
}{
	TenantIdTenant:          "TenantIdTenant",
	PermissionIdPermissions: "PermissionIdPermissions",
...
type roleR struct {
	...
	PermissionIdPermissions PermissionSlice `boil:"PermissionIdPermissions" json:"PermissionIdPermissions" toml:"PermissionIdPermissions" yaml:"PermissionIdPermissions"`
	UserIdUsers             UserSlice       `boil:"UserIdUsers" json:"UserIdUsers" toml:"UserIdUsers" yaml:"UserIdUsers"`
}
...
func (r *roleR) GetTenantIdTenant() *Tenant {
	if r == nil {
		return nil
	}
...
func (r *roleR) GetPermissionIdPermissions() PermissionSlice {
	if r == nil {
		return nil
	}
	return r.PermissionIdPermissions
}

func (r *roleR) GetUserIdUsers() UserSlice {
	if r == nil {
		return nil
	}
	return r.UserIdUsers
}
...
var (
	roleAllColumns            = []string{"id", "name", "tenantId", "createdAt", "updatedAt"}
	roleColumnsWithoutDefault = []string{"id", "tenantId"}
	roleColumnsWithDefault    = []string{"name", "createdAt", "updatedAt"}
...
func (o *Role) TenantIdTenant(mods ...qm.QueryMod) tenantQuery {
...
// PermissionIdPermissions retrieves all the permission's Permissions with an executor via id column.
func (o *Role) PermissionIdPermissions(mods ...qm.QueryMod) permissionQuery {
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"role_permissions\" on \"permissions\".\"id\" = \"role_permissions\".\"permissionId\""),
		qm.Where("\"role_permissions\".\"roleId\"=?", o.ID),
	)
...
func (o *Role) UserIdUsers(mods ...qm.QueryMod) userQuery {
	var queryMods []qm.QueryMod
	if len(mods) != 0 {
		queryMods = append(queryMods, mods...)
	}
	...
	queryMods = append(queryMods,
		qm.InnerJoin("\"user_roles\" on \"users\".\"id\" = \"user_roles\".\"userId\""),
		qm.Where("\"user_roles\".\"roleId\"=?", o.ID),
	)
...
func (roleL) LoadTenantIdTenant(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	if singular {
		...
		foreign.R.TenantIdRoles = append(foreign.R.TenantIdRoles, object)
		return nil
	}
	...
	for _, local := range slice {
		for _, foreign := range resultSlice {
			if local.TenantId == foreign.ID {
				...
				foreign.R.TenantIdRoles = append(foreign.R.TenantIdRoles, local)
				break
			}
		}
	}
...
// LoadPermissionIdPermissions allows an eager lookup of values, cached into the
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (roleL) LoadPermissionIdPermissions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(
		qm.Select("\"permissions\".\"id\", \"permissions\".\"name\", \"permissions\".\"description\", \"permissions\".\"actionId\", \"permissions\".\"resourceId\", \"permissions\".\"tenantId\", \"permissions\".\"createdAt\", \"permissions\".\"updatedAt\", \"a\".\"roleId\""),
		qm.From("\"permissions\""),
		qm.InnerJoin("\"role_permissions\" as \"a\" on \"permissions\".\"id\" = \"a\".\"permissionId\""),
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.Description, &one.ActionId, &one.ResourceId, &one.TenantId, &one.CreatedAt, &one.UpdatedAt, &localJoinCol)
		if err != nil {
	...
	if singular {
		object.R.PermissionIdPermissions = resultSlice
		for _, foreign := range resultSlice {
			if foreign.R == nil {
	...
	for i, foreign := range resultSlice {
		...
		for _, local := range slice {
			if local.ID == localJoinCol {
				local.R.PermissionIdPermissions = append(local.R.PermissionIdPermissions, foreign)
				if foreign.R == nil {
					foreign.R = &permissionR{}
				}
...
// loaded structs of the objects. This is for a 1-M or N-M relationship.
func (roleL) LoadUserIdUsers(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRole interface{}, mods queries.Applicator) error {
	var slice []*Role
	var object *Role
	...
	query := NewQuery(
		qm.Select("\"users\".\"id\", \"users\".\"name\", \"users\".\"email\", \"users\".\"emailVerified\", \"users\".\"tenantId\", \"users\".\"createdAt\", \"users\".\"updatedAt\", \"a\".\"roleId\""),
		qm.From("\"users\""),
		qm.InnerJoin("\"user_roles\" as \"a\" on \"users\".\"id\" = \"a\".\"userId\""),
		qm.WhereIn("\"a\".\"roleId\" in ?", argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	...
	for results.Next() {
		...
		err = results.Scan(&one.ID, &one.Name, &one.Email, &one.EmailVerified, &one.TenantId, &one.CreatedAt, &one.UpdatedAt, &localJoinCol)
		if err != nil {
...
// SetTenantIdTenant of the role to the related item.
// Sets o.R.TenantIdTenant to related.
// Adds o to related.R.TenantIdRoles.
func (o *Role) SetTenantIdTenant(ctx context.Context, exec boil.ContextExecutor, insert bool, related *Tenant) error {
	var err error
	if insert {
	...
	if related.R == nil {
		related.R = &tenantR{
			TenantIdRoles: RoleSlice{o},
		}
	} else {
		related.R.TenantIdRoles = append(related.R.TenantIdRoles, o)
	}

	return nil
}

// AddPermissionIdPermissions adds the given related objects to the existing relationships
// of the role, optionally inserting them as new records.
// Appends related to o.R.PermissionIdPermissions.
// Sets related.R.RoleIdRoles appropriately.
func (o *Role) AddPermissionIdPermissions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Permission) error {
	var err error
	...
	for _, rel := range related {
		query := "insert into \"role_permissions\" (\"roleId\", \"permissionId\") values ($1, $2)"
	...
	if o.R == nil {
		o.R = &roleR{
			PermissionIdPermissions: related,
		}
	} else {
		o.R.PermissionIdPermissions = append(o.R.PermissionIdPermissions, related...)
	}
...
// SetPermissionIdPermissions removes all previously related items of the
// role replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.RoleIdRoles's PermissionIdPermissions accordingly.
// Replaces o.R.PermissionIdPermissions with related.
// Sets related.R.RoleIdRoles's PermissionIdPermissions accordingly.
func (o *Role) SetPermissionIdPermissions(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*Permission) error {
	query := "delete from \"role_permissions\" where \"roleId\" = $1"
	...
	removePermissionIdPermissionsFromRoleIdRolesSlice(o, related)
	if o.R != nil {
		o.R.PermissionIdPermissions = nil
	}

	return o.AddPermissionIdPermissions(ctx, exec, insert, related...)
}

// RemovePermissionIdPermissions relationships from objects passed in.
// Removes related items from R.PermissionIdPermissions (uses pointer comparison, removal does not keep order)
// Sets related.R.RoleIdRoles.
func (o *Role) RemovePermissionIdPermissions(ctx context.Context, exec boil.ContextExecutor, related ...*Permission) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"role_permissions\" where \"roleId\" = $1 and \"permissionId\" in (%s)",
	...
	for _, rel := range related {
		for i, ri := range o.R.PermissionIdPermissions {
			if rel != ri {
				continue
			}

			ln := len(o.R.PermissionIdPermissions)
			if ln > 1 && i < ln-1 {
				o.R.PermissionIdPermissions[i] = o.R.PermissionIdPermissions[ln-1]
			}
			o.R.PermissionIdPermissions = o.R.PermissionIdPermissions[:ln-1]
			break
		}
	}
...
func removePermissionIdPermissionsFromRoleIdRolesSlice(o *Role, related []*Permission) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
...
// Sets related.R.RoleIdRoles appropriately.
func (o *Role) AddUserIdUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	var err error
	for _, rel := range related {
		if insert {
			if err = rel.Insert(ctx, exec, boil.Infer()); err != nil {
	...
	for _, rel := range related {
		query := "insert into \"user_roles\" (\"roleId\", \"userId\") values ($1, $2)"
		values := []interface{}{o.ID, rel.ID}
...
// role replacing them completely with the passed
// in related items, optionally inserting them as new records.
// Sets o.R.RoleIdRoles's UserIdUsers accordingly.
// Replaces o.R.UserIdUsers with related.
// Sets related.R.RoleIdRoles's UserIdUsers accordingly.
func (o *Role) SetUserIdUsers(ctx context.Context, exec boil.ContextExecutor, insert bool, related ...*User) error {
	query := "delete from \"user_roles\" where \"roleId\" = $1"
	values := []interface{}{o.ID}
	if boil.IsDebug(ctx) {
		writer := boil.DebugWriterFrom(ctx)
		fmt.Fprintln(writer, query)
	...
	removeUserIdUsersFromRoleIdRolesSlice(o, related)
	if o.R != nil {
		o.R.UserIdUsers = nil
	}
...
// Sets related.R.RoleIdRoles.
func (o *Role) RemoveUserIdUsers(ctx context.Context, exec boil.ContextExecutor, related ...*User) error {
	if len(related) == 0 {
		return nil
	}
	...
	var err error
	query := fmt.Sprintf(
		"delete from \"user_roles\" where \"roleId\" = $1 and \"userId\" in (%s)",
		strmangle.Placeholders(dialect.UseIndexPlaceholders, len(related), 2, 1),
	)
	values := []interface{}{o.ID}
	for _, rel := range related {
...
func removeUserIdUsersFromRoleIdRolesSlice(o *Role, related []*User) {
	for _, rel := range related {
		if rel.R == nil {
			continue
		}
