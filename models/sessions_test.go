...
func testSessionToOneUserUsingUserIdUser(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadUserIdUser(ctx, tx, false, (*[]*Session)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testSessionToOneSetOpUserUsingUserIdUser(t *testing.T) {
	var err error
...
var (
	sessionDBTypes = map[string]string{`ID`: `character varying`, `UserId`: `character varying`, `Token`: `character varying`, `ExpiresAt`: `timestamp without time zone`, `CreatedAt`: `timestamp without time zone`}
	_              = bytes.MinRead
)
