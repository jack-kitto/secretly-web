...
func testEnvironmentToOneTenantUsingTenantIdTenant(t *testing.T) {
	ctx := context.Background()
	tx := MustTx(boil.BeginTx(ctx, nil))
	defer func() { _ = tx.Rollback() }()
	...
	if err = local.L.LoadTenantIdTenant(ctx, tx, false, (*[]*Environment)(&slice), nil); err != nil {
		t.Fatal(err)
	}
...
func testEnvironmentToOneSetOpTenantUsingTenantIdTenant(t *testing.T) {
	var err error
	...
	for i, x := range []*Tenant{&b, &c} {
		...
		if x.R.TenantIdEnvironments[0] != &a {
...
var (
	environmentDBTypes = map[string]string{`ID`: `character varying`, `Name`: `character varying`, `TenantId`: `character varying`, `CreatedAt`: `timestamp without time zone`, `UpdatedAt`: `timestamp without time zone`}
	_                  = bytes.MinRead
)
